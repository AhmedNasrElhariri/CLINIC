...
ssh root@159.223.18.82 "cd clinicr &&
 ...
 file_dir_name=~/uploads_backups/"$(date +"%d-%m-%Y  %H:%M")" && mkdir "${file_dir_name}" &&  cp -a  ~/clinicr/server-dist/uploads/. "${file_dir_name}" && \
 mkdir ~/"${file_dir_name}" &&  cp -a  ~/clinicr/server-dist/uploads/. ~/"${file_dir_name}"/
